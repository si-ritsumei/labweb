fetch("../json/lablife.json")
  ...
  .then(function (jsonData) {
    ...
    for (var label in jsonData) {
      ...
      for (var article of jsonData[label]) {
        var articles_ofyear = {
          ...
          month: article.month,
          date: article.date,
          title: checkLangage(lang, article.title)
            ? checkLangage(lang, article.title)
            : null,
    ...
    articles.forEach(function (article) {
      ...
      img.src = "img/eventNews/" + article.year + "/" + article.image;
      img.style.width = "100%";
...
function create_date(lang, article) {
  if (lang === "ja") {
    ...
    return (
      month_en(article.month) +
      " " +
      article.date +
      day_sign(article.date) +
      ", " +
      article.year
    );
  }
