...
> キャメルケース：変数の最初の文字は小文字で"\_"を使わずに単語を繋ぐ。各単語の頭を大文字、他を小文字にする形式。

### 定数にはアッパースネークケースを採用

`教授名：村上陽平`といった定数を作る場合、`PROFESSOR_NAME`といった定数名の記述になる

> アッパースネークケース：全ての文字を大文字で、単語間を"\_"を使って繋ぐ形式

### 関数名はキャメルケースを採用

関数名は動詞で始まる名前を意識。名前を取得する関数なら`getName()`といった関数名になる

### クラス名はアッパーキャメルケースを採用

`ClassNameExample`といったクラス名になる

> アッパーキャメルケース：単語の頭文字を大文字で記述する形式

### ファイル名はケバブケースを採用

`labweb-index-example.html`といったファイル名の記述になる

> ケバブケース：全ての文字を小文字で、単語間を"-"を使って繋ぐ形式
